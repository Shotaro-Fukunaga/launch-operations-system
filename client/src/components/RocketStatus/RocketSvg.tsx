...
const RocketComponent: React.FC<Props> = ({
  ...
  const determineColor = (status: number): string => {
    switch (status) {
      case 0:
        return "#808080BF";
      case 1:
  ...
  // 色を決定
  const fairingFill = determineColor(fairingStatus);
  const firstStageFill = determineColor(firstStageStatus);
  const secondStageFill = determineColor(secondStageStatus);
  return (
    <svg
      width="100%"
      height="100%"
